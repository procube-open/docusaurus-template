...
def main():
    ...
    for root, dirs, files in os.walk(repo_root):
        ...
        for file in files:
            ...
            # ファイル名にパッケージ名が含まれている場合はリネーム
            if DEFAULT_PACKAGE in file:
                new_file_name = file.replace(DEFAULT_PACKAGE, new_package)
                new_file_path = Path(root) / new_file_name
                try:
                    file_path.rename(new_file_path)
                    rel_path_old = file_path.relative_to(repo_root)
                    rel_path_new = new_file_path.relative_to(repo_root)
                    renamed_files.append((rel_path_old, rel_path_new))
                    print(f"🔄 {rel_path_old} → {rel_path_new}")
                    # リネーム後のパスで処理を続ける
                    file_path = new_file_path
                except Exception as e:
                    rel_path = file_path.relative_to(repo_root)
                    print(f"✗ {rel_path} のリネーム失敗: {e}", file=sys.stderr)
                    continue
    ...
    print("=" * 60)
    print(f"置き換えが完了しました！")
    print(f"  - {len(renamed_files)}個のファイルをリネーム")
    print(f"  - {len(modified_files)}個のファイルを更新")
    print("=" * 60)
